import { Activity, Star, People, Bullseye, Fire, GraphUpArrow } from "react-bootstrap-icons";
      ...
      {stats && (
        <>
          <div className="stat-tiles">
            ...
            <StatTile icon={<People size={20} />} value={stats.nb_ranked_players} label="Joueurs classés" tint="lime" />
          </div>
          ...
          <div className="card">
            {stats.most_regular && <div className="list-row"><span style={{ display: "flex", alignItems: "center", gap: 7 }}><Bullseye size={14} />Joueur le plus régulier</span><strong>{stats.most_regular.name}</strong></div>}
            ...
            {stats.best_progression && <div className="list-row"><span style={{ display: "flex", alignItems: "center", gap: 7 }}><GraphUpArrow size={14} />Meilleure progression</span><strong>{stats.best_progression.name} (<span className="num">{stats.best_progression.value > 0 ? "+" : ""}{fmtScore(stats.best_progression.value)}</span>)</strong></div>}
          </div>
        </>
      )}
    </div>
